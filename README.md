...
PyDOS currently supports these commands:
...
- editing a .txt or a .pds file that exists within the directory PyDOS is in 
- scripts, with a .pds extension (short for PyDosScript), that are written on your host PC, or through the edit feature in PyDOS

## Overview


- Virtual file system with hierarchical folder structure
- Multi-user system with admin and normal roles
- Persistent state storage using JSON
- Kernel-style bugcheck recovery for corrupted state files
- Command parser and dispatcher system

## Notes
The defaut account is ```admin```, with the password ```root```.  
The login screen is CASE SENSITIVE, the shell is CASE INSENSITIVE  
If ```state.json``` is corrupted, a bugcheck will be raised. ```state.json``` will be renamed to ```state CORRUPTED.json```, and a new clean ```state.json``` will be generated.
